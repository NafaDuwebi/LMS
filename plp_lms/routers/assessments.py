...
def create_assessment(
    ...
    randomise_questions: bool = Form(False),
    randomise_options: bool = Form(False),
    release_results_immediately: bool = Form(True),
    ...
    assessment = Assessment(
        ...
        release_results_immediately=release_results_immediately,
    )
    db.add(assessment)
    db.commit()
...
def marking_queue(request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if user.role == "superadmin":
        submissions = db.query(Submission).filter(Submission.status == "submitted").order_by(Submission.submitted_at).all()
    elif user.role == "trainer":
        from models.cohort import Cohort
        course_ids = db.query(Cohort.course_id).filter(Cohort.trainer_id == user.id).distinct()
        submissions = db.query(Submission).join(Submission.assessment).filter(
            Submission.status == "submitted",
            Assessment.course_id.in_(course_ids),
        ).order_by(Submission.submitted_at).all()
    else:
        submissions = []
    return templates.TemplateResponse("shared/marking_queue.html", {"request": request, "user": user, "submissions": submissions})
...
def view_assessment(request: Request, assessment_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
